--------------------------------------------
--https://doxbin.com/upload/bacondoxx/raw---
--------------------------------------------



if game.Players.LocalPlayer.UserId == 3450807169 then
    game.Players.LocalPlayer:Kick("Blacklisted. Reason: Skidding my scripts.") -- 143.44.164.57 + Ratio --
end

if game.Players.LocalPlayer.UserId == 3507271874 then
    game.Players.LocalPlayer:Kick("Blacklisted. Reason: Skidding my scripts.") -- 143.44.164.57 + Ratio --
end

if game.Players.LocalPlayer.UserId == 3688478116 then
    game.Players.LocalPlayer:Kick("Blacklisted. Reason: Skidding my scripts.") -- 143.44.164.57 + Ratio --
end

if game.Players.LocalPlayer.UserId == 3574371473 then
    game.Players.LocalPlayer:Kick("Blacklisted. Reason: Skidding my scripts.") -- 143.44.164.57 + Ratio --
end

if game.Players.LocalPlayer.UserId == 3593481762 then
    game.Players.LocalPlayer:Kick("Blacklisted. Reason: Skidding my scripts.") -- 143.44.164.57 + Ratio --
end



--------------------------------------------
-------------insert dx in here--------------
--------------------------------------------
